...
image_paths_and_description_file = [(".\\dataset\\NUOVO\\val",
                                     ...
                                     ".\\dataset\\NUOVO\\val\\output_segmentationv2.csv"),
                                    (".\\dataset\\NUOVO\\train",
                                     ".\\dataset\\NUOVO\\train\\output.csv",
                                     ".\\dataset\\NUOVO\\train\\output_final.csv",
                                     ".\\dataset\\NUOVO\\train\\output_segmentationv2.csv"),
                                    (".\\dataset\\NUOVO\\test",
                                     ".\\dataset\\NUOVO\\test\\output.csv",
                                     ".\\dataset\\NUOVO\\test\\output_final.csv",
                                     ".\\dataset\\NUOVO\\test\\output_segmentationv2.csv")
                                    ]
...
def calculate_box(x, y, w, h):
    ...
    for i in range(BLOCKS_NUM):
        for j in range(BLOCKS_NUM):
            if i * BLOCK_SIZE <= centerx and ((i + 1) * BLOCK_SIZE) >= centerx and j * BLOCK_SIZE <= \
                    centery and ((j + 1) * BLOCK_SIZE) >= centery:
                # print(x, y, w, h)
                # print(x+w, y+h)
                # print(centerx, centery)
                # print(i * BLOCK_SIZE, j * BLOCK_SIZE)
                # print((i+1) * BLOCK_SIZE, (j+1) * BLOCK_SIZE)
                output += "1," + str(centerx) + "," + str(centery) + "," + str(w) + "," + str(h) + ","
            else:
...
if __name__ == '__main__':
    ...
    for el in image_paths_and_description_file:
        ...
        for index, row in df.iterrows():
            ...
            calculate_box(row["x_1"], row["y_1"], row["width"], row["height"])
